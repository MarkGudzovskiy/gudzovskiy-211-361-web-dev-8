...
function autoCompleteItem(event) {
    let searchField = document.querySelector('.search-field');
    searchField.value = event.target.innerHTML;  // вставляем в поле
    let autoCompleteList = document.querySelector('.autocomplete');
    autoCompleteList.innerHTML = '';

    downloadData();  // чтобы по нажатию на автозаполнение сработал поиск
...
// просто чтобы вызвать функцию полностью Задание 1
function SearchdownloadData() {
    downloadData();

    // чтобы убрать список атодополнения  (если такой есть) при нажатии на поиск
    let autoCompleteList = document.querySelector('.autocomplete');
    autoCompleteList.innerHTML = '';
}
